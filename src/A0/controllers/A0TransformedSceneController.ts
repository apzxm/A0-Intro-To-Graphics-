...
export class A0TransformedSceneController extends TransformVisSceneController{
        ...
        if(nodeModel instanceof AxesModel){
            return AxesView.Create(nodeModel,
                (view:AxesView)=>{
                    ...
                    view.setTargetVector(V2(), axisTransform.times(view.model.targetPoint));
    ...
    onClick(event:AInteractionEvent){
        ...
        if(cursorPosition) {
            if (this.keyboardInteraction.keysDownState['x']) {
                ...
                this.model.targetPoint = axisTransform.getInverse()!.times(cursorPosition);
            }
            if(event.shiftKey){
